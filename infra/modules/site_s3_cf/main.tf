...
locals {
  bucket_name = lower(replace("${var.name}-site-${data.aws_caller_identity.current.account_id}", "_", "-"))
}
...
# Allow CloudFront (via OAC) to read from bucket
...
# Upload all files from site_dir
locals {
